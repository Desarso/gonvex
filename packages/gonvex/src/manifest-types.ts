...
/** Prebuilt JavaScript for runtimes that execute the module directly. */
export type ModuleJavaScript = {
  /** Project-relative POSIX path the bundle was read from. */
  path: string;
  hash: string;
  /** base64-encoded bundle contents. */
  code: string;
  /** base64-encoded source map when one sits next to the bundle. */
  sourceMap?: string;
};
