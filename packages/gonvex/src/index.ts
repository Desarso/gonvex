...
function renderAPI(manifest: Manifest) {
  const lines = ["// Generated by gonvex dev. Do not edit.", "", "export const api = {"];
  for (const [path, entry] of Object.entries(manifest.functions).sort(([a], [b]) => a.localeCompare(b))) {
    lines.push(`  ${JSON.stringify(path)}: { kind: ${JSON.stringify(entry.kind)}, path: ${JSON.stringify(path)} },`);
  }
  lines.push("} as const;", "", "export type Api = typeof api;", "");
  return lines.join("\n");
}
