...
Use indexes for Queries and Live Query plans that filter, sort, search, or page through large tables.
...
tenant.tables.tasks;
controlPlane.tables.projects;
```

Generated schema modules currently expose table/column/index metadata and table
name types. Full document-row type inference from Go structs is not implemented.
