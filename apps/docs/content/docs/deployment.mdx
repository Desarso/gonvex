...
only to the Whagons development Coolify service stack. It updates both remote Git
build contexts in that stack to the exact commit SHA, writes that SHA to
`GONVEX_RUNTIME_VERSION`, verifies the saved Compose definition, and then queues
the development deployment. Pull requests run the same regression suite but never
deploy.
...
The workflow reads `COOLIFY_API_URL` and `COOLIFY_API_TOKEN` from the protected
`gonvex-runtime` GitHub environment. Keep the token there rather than in the
repository. The exact SHA pin makes the running source auditable and avoids a
moving-branch race during a rebuild.
