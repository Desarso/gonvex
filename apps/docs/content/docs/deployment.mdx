...
The `Runtime regression` GitHub workflow deploys every successful push to `main`
only to the Whagons development runtime and dashboard Applications. Both use the
Dockerfile build pack, have `/healthz` checks, and are pinned to the exact tested
commit. Coolify starts the replacement, waits for readiness, moves traffic, and
then stops the previous container. The runtime deploy finishes before the dashboard
deploy starts. Pull requests run the same regression suite but never deploy.
...
`/var/lib/gonvex` is intentionally container-local: source manifests live in
Postgres, data files rehydrate from object storage, and sharing Go plugin caches
between two runtime binaries would make a rolling overlap unsafe.
...
use renewable row leases for crash recovery. During upgrades from the earlier
Valkey-only scheduler, Valkey claim keys remain as a compatibility fence and the
new runtime drains old queued jobs into Postgres. HTTP and WebSocket traffic can
therefore use both rolling containers while either version safely schedules or
executes work without duplicating a cron fire.
...
verified backup or restore-point reference. The workflow requires successful CI,
...
service and are not restarted. Public health checks must report the exact runtime
SHA before the workflow fast-forwards `production` without force and creates the
immutable `gonvex-prod-<sha>` release tag. A failed rollout leaves the production
branch and release tag at the last verified deployment. A push to `main` never
...
The deploy verifier also receives an explicit expected runtime auth mode. The
development lane requires `GONVEX_REQUIRE_AUTH=true`. Production currently
requires `GONVEX_REQUIRE_AUTH=false` because Skills Vault still performs its own
`auth.login` bootstrap over a WebSocket before it has a Gonvex-native
`gvx_session_` token. This compatibility mode does not disable configured
Firebase verification or per-project native auth; those project-specific checks
still take precedence. Migrate Skills Vault to Gonvex native app auth before
changing the production expectation to `true`.

The data-plane Compose attaches Postgres and Valkey to the shared `coolify`
network with stable `gonvex-postgres` and `gonvex-valkey` aliases. Tenant
database URLs stored in the registry use those names, so deployments must retain
the aliases whenever the data containers are recreated.
