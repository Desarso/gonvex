...
hydrated. During the short rolling overlap, a PostgreSQL advisory lock elects one
scheduler leader so HTTP and WebSocket traffic can use both containers without
executing cron jobs twice.
