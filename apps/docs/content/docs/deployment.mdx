...
to the Whagons development and production Coolify service stacks. It updates both
remote Git build contexts to the exact commit SHA, writes that SHA to
`GONVEX_RUNTIME_VERSION`, verifies the saved Compose definition, and then queues
the deployments. Pull requests run the same regression suite but never deploy.
