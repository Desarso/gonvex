...
hydrated. Scheduled jobs use a shared Valkey queue with renewable execution
leases, crash recovery, and deterministic cron-occurrence IDs. HTTP and WebSocket
traffic can therefore use both rolling containers while any replica safely
enqueues or executes scheduled work without losing or duplicating a cron fire.

Production is a separate promotion lane. After the development release passes the
Whagons Monitor verification at `https://monitor.whagons.com/releases`, promote
that exact Gonvex SHA to the production branch and deploy only the production
Coolify service. A push to `main` must never update the production runtime
directly.
...
environment. Keep the token there rather than in the repository. The exact SHA
pin makes the running source auditable and avoids a moving-branch race during a
rebuild.
