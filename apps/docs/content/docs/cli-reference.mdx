...
The default callback path is `/auth/callback`; override it with
...
`GoogleSignInButton`, and `useGonvexAuth` exports.
...
Disable new Google sign-ins with `gonvex auth remove google`. Existing account and
session records are retained; disabling the provider prevents new authorization
flows. The Gonvex runtime operator must configure one central Google OAuth client as
described in [Self-hosting and Deployment](./deployment#one-time-google-auth-broker-setup).

Native Google login currently applies to `single` database-mode projects. The
runtime refuses to enable it on `multiTenant` projects until tenant membership can
