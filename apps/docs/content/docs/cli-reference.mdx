...
TypeScript Actions read these values through the immutable `ctx.env` object.
Queries and Reducers do not receive environment variables, keeping transactional
code isolated from host secrets and external configuration.
