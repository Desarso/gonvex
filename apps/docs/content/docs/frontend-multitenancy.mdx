...
The frontend can choose a tenant, but it is not trusted as authority. The runtime verifies membership before executing Queries, Reducers, Actions, Live Queries, or Replica Collections.
...
For production apps, always send a real auth token too. The runtime rejects
tenant access unless the authenticated user is a member of that tenant.
...
The open socket reauthenticates. The runtime detaches and reattaches active live
Live Queries with the new verified tenant, resets Replica Collections when the
visibility scope changes, and issues a new cache scope. React query/sync hooks
clear their previous scoped result until the new scope produces local or server
data. Normalized server entities remain owned by the Local Replica rather than
application React state.
...
Remount or clear any additional tenant-specific state owned by your application;
the Gonvex client can only clear the query/sync state it manages.

## Control Plane Data vs Tenant Data
...
Tenant list and membership screens should come from Control Plane/project-level functions or runtime endpoints. Tenant business screens should run under the active tenant context.

## Generated Table Scopes

The generated frontend folder separates Control Plane and tenant table metadata:

```txt
gonvex/_generated/
  schema.ts
  control-plane/tables.ts
  tenant/tables.ts
```

Use the tenant metadata for business-data UI and the Control Plane metadata for app-owned project/control-plane UI:

```ts
import { controlPlane, tenant } from "./gonvex/_generated/schema";
import { tables as controlPlaneTables } from "./gonvex/_generated/control-plane/tables";
import { tables as tenantTables } from "./gonvex/_generated/tenant/tables";

const tenantTableNames = Object.keys(tenantTables);
const controlPlaneTableNames = Object.keys(controlPlaneTables);
```
...
- `Control Plane / project DB` means no tenant id is attached.
...
- Use Control Plane/project data only for routing, membership, and tenant selection; keep business data tenant-local.
