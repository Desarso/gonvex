...
Application modules do not register HTTP functions. Browser and mobile code
invoke Queries, Reducers, and Actions through the authenticated runtime
protocol. An infrastructure adapter receiving an external webhook must verify
and normalize the provider request before invoking an Action; durable changes
then re-enter through a Reducer.
...
If every tenant has a separate database, the runtime still needs a central place to know that the same user can access all three tenants. That belongs in the Control Plane database.
...
Personal access tokens are stored hashed in the Control Plane database, belong to one
...
Functions receive the verified user and permission payload in their runtime
context. Apply the project's permission policy before accessing data:
