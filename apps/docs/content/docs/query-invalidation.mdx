...
For example, the runtime treats `workspace_id`, `organization_id`, or
`account_id` as ordinary predicates. Those names are not hardcoded into Gonvex
core.
...
Broad refresh only affects active subscriptions in the relevant
project/tenant/database scope.
...
Writes in tenant `acme` do not refresh subscriptions in tenant `globex`.
Landlord/project data uses a separate scope.
...
Durable sync collections use their own transactional change log rather than this
snapshot-only recovery path. See [Durable Sync Collections](./durable-sync).
