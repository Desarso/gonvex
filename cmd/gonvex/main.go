...
func parseSchema(file string) (map[string]manifest.Table, error) {
	source, err := os.ReadFile(file)
	if err != nil {
		return nil, err
	}
	...
	tablePattern := regexp.MustCompile(`(?s)s\.Table\(\s*"([^"]+)"\s*,\s*func\([^)]*\)\s*\{(.*?)\n\s*\}\s*\)`)
	columnPattern := regexp.MustCompile(`t\.(ID|String|Text|Int|Int64|Float64|Bool|Time|JSON)\(\s*"([^"]+)"([^)]*)\)`)
	indexPattern := regexp.MustCompile(`t\.(Index|UniqueIndex)\(\s*"([^"]+)"([^)]*)\)`)
...
func writeBindings(root string, m manifest.Manifest) error {
	...
	files := map[string]string{
		...
		"schema.ts":     "// Generated by gonvex dev. Do not edit.\nexport const schema = {} as const;\n",
	}
...
func renderAPI(m manifest.Manifest) string {
	...
	for path, entry := range m.Functions {
