...
func TestPropagationAggregatorComputesTimeToLastUserByReducerPath(t *testing.T) {
	...
	if report.CommittedReducers != 4 || report.CommitsWithPropagation != 3 || report.CommitsWithoutPropagation != 1 {
	...
	create := report.ByReducerPath["tasks.create"]
	...
	update := report.ByReducerPath["tasks.update"]
	if update.Commits != 1 || update.PropagationSamples != 2 || update.TTLU.MaxMS != 15 {
		t.Fatalf("unexpected tasks.update TTLU: %#v", update)
	}
}

func TestPropagationAggregatorCorrelatesOriginCommandIDsAcrossTimestampSkewAndCoalescing(t *testing.T) {
	aggregator := newPropagationAggregator()
	// A delivery may race ahead of the reducer response on another socket.
	aggregator.RecordOriginCommandDeliveries([]string{"command-a", "command-b"}, "client-a", 1050)
	aggregator.RecordCommandCommit("command-a", 1000, "tasks.create")
	aggregator.RecordCommandCommit("command-b", 1020, "tasks.update")
	aggregator.RecordOriginCommandDeliveries([]string{"command-a", "command-b"}, "client-b", 1080)

	report := aggregator.Report()
	if report.CommittedReducers != 2 || report.CommitsWithPropagation != 2 || report.PropagationSamples != 4 {
		t.Fatalf("unexpected ID-correlated commit counts: %#v", report)
	}
	if got := report.ByReducerPath["tasks.create"].TTLU.MaxMS; got != 80 {
		t.Fatalf("tasks.create TTLU = %v, want 80", got)
	}
	if got := report.ByReducerPath["tasks.update"].TTLU.MaxMS; got != 60 {
