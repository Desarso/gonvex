...
type Profile struct {
	...
	Reducers                   []ReducerSpec      `json:"reducers,omitempty"`
}
...
func loadProfileReader(reader io.Reader) (Profile, error) {
	...
	for index := range profile.Reducers {
		spec := &profile.Reducers[index]
		if spec.Args == nil {
			spec.Args = map[string]any{}
		}
		if err := validateOperation(spec.Path, spec.Args, "reducer", index); err != nil {
			return Profile{}, err
		}
		if spec.RatePerUserPerMinute < 0 || spec.RatePerMinute < 0 {
			return Profile{}, fmt.Errorf("reducer %d (%s) rates cannot be negative", index, spec.Path)
		}
		if spec.RatePerUserPerMinute == 0 && spec.RatePerMinute == 0 {
			return Profile{}, fmt.Errorf("reducer %d (%s) must define a positive rate", index, spec.Path)
		}
		if spec.ActiveUsers == 0 {
			spec.ActiveUsers = 1
		}
		if spec.ActiveUsers < 0 || spec.ActiveUsers > 1 {
			return Profile{}, fmt.Errorf("reducer %d (%s) activeUsers must be between 0 and 1", index, spec.Path)
		}
		spec.pools = clonePools(profile.Pools)
	}
...
func (s ReducerSpec) expandedArgs(runtimeVariables map[string]any) (map[string]any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("expanded reducer args are not an object")
	}
	return args, nil
}
