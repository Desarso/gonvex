...
function resultMessage(client, id, reason) {
  ...
  return {
    ...
    id,
    path: "replica.demo",
    result: [{ id: "demo", value: state.value }],
    reason,
    replicaScope: client.scope,
    windowRevision: nextRevision(),
    subscriptionRevision: { epoch: "runtime", sequence: revision },
  };
}
...
webSockets.on("connection", (socket, request) => {
  ...
  socket.send(JSON.stringify({
    type: "session.ready",
    project: "local-replica-test",
    tenant: "browser",
    replica: {
      protocolVersion: 1,
      scope: client.scope,
      visibilityScope: client.scope,
      epoch: "e".repeat(64),
    },
  }));
...
server.listen(port, host, () => {
  process.stdout.write(`local replica test server listening on http://${host}:${port}\n`);
});
