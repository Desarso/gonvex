...
async function control(channel: string, options: { value?: string; delayMs?: number; broadcast?: boolean }) {
  ...
  if (!response.ok) throw new Error(`Local Replica test control failed: ${response.status}`);
}

test.describe("persistent Local Replica Live Query", () => {
  ...
  test("persists committed-change results for the next reload", async ({ page }, testInfo) => {
    const channel = `change-${testInfo.workerIndex}-${Date.now()}`;
    const scope = testScope(channel);
    await control(channel, { value: "initial", delayMs: 0 });
    await page.goto(`/?channel=${channel}&scope=${scope}`);
    await expect(page.getByTestId("query-value")).toHaveText("initial");

    await control(channel, { value: "changed", broadcast: true });
    await expect(page.getByTestId("query-value")).toHaveText("changed");
    await page.waitForTimeout(150);

    await control(channel, { value: "after-reload", delayMs: 700 });
    await page.reload();
    await expect(page.getByTestId("query-value")).toHaveText("changed", { timeout: 400 });
